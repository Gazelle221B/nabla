...
// (ギブス現象、ジャンプ幅の約9%のオーバーシュート)は N をどれだけ増やしても消えない——
...
/** t をπの倍数として表示するための整形(表示専用)。 */
function formatAsPiMultiple(t: number): string {
	return `${round2(t / Math.PI)}π`;
}
		...
		>
			...
			<noscript>
				<p className={styles.noscript}>
					...
					増やすほど波形は方形波に近づきますが、不連続点のすぐそばには高さ約1.18(ジャンプの
					約9%分の飛び出し)の「ツノ」が残り続け、N をどれだけ増やしても消えません(ギブス
				...
				<>
					...
					<div className={styles.observation} aria-live="polite">
						...
						<p className={styles.statusNeutral}>
							N を大きくしても、S{n} の最大値は約1.18のまま1へは戻っていません(ギブス現象)。
						</p>
					</div>
					...
					<div className={styles.checkpoint}>
						...
						<p>
							{predictionCorrect
								? 'その通りです。'
								: '実は、N をどれだけ増やしても、不連続点のそばには高さ約1.18(ジャンプ幅の約9%分の飛び出し)の「ツノ」が残り続け、消えません。予想と見比べてみましょう。'}
						</p>
						<p className={styles.narration}>
							...
							幅は狭くなっていきますが、その高さはどこまでいっても約1.18(ジャンプ幅2の約8.95%
							分のオーバーシュート)のまま一定で、1へは戻りません。これを<strong>ギブス現象</strong>と
						...
						<p className={styles.narration}>
							...
							ツノの幅はNを増やすと0に近づきますが、高さは常に約1.18のまま残るため、
