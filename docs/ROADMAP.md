...
> **✅ 2026-07-14: 全マイルストーン完走(MVP 1〜3+M4〜M8)。** MVP 2(PR #37〜#39: フーリエ〔計測により Tier 1〕・LLN/CLT〔Pixi〕・フラクタル〔Canvas2D〕)、MVP 3(PR #41〜#44: 一次変換3×3・回転と基底変換・曲面と偏微分・ドメインカラーリング〔ShaderMaterial、Tier 3b 不要を実測確定〕)。公開32単元(当初「33」と記録したが sample-math 込みの誤計上、PR #48 時に是正)。
...
> **✅ 2026-07-24: 全マイルストーン完走を受け、次フェーズは M9「計測と構造化」(ADR-006、Issue #49)。** 重心を「生産」から「検証・構造化・流通」へ移す。M9a(GA4 計測基盤+事前登録基準)→ M9b(前提チェック関門パイロット)→ M9c(演習+予想履歴)→ M9d(既定学習経路・伴走者ガイド・印刷ワークシート・URL 初期状態固定)。**サブ波の正確な範囲・完了条件は ADR-006 を SSOT とする(本欄は要約)。****M9 期間中、新単元の追加は前提グラフの空洞埋め・経路整備に必要な場合に限定する(単元数を KPI にしない)。** 旧次アクション(M4 着手以降)は完了済みのため歴史的記録として下記に残す。

1. ~~**M4 着手(本ロードマップ確定後)**: `linear-function` → `quadratic-function` → `trigonometric-ratios` の順に前提を満たしつつ実装(各 PR)。並行して rule of three の共有枠組み抽出を M4 内で行う。~~(完了)
2. 各単元は「この調子」(委譲実装 → 別系統レビュー → Antigravity 数学QA → CI → マージ)で完成させ、`PROJECT_STATE` を随時更新。(M9 でも踏襲)
3. ~~M4 完了後に M5 を波として具体化。MVP 2 / MVP 3 は Tier 導入 ADR + 人間 Go を経て着手。~~(完了)
...
| 2026-07-24 | 全マイルストーン完走を受け §4 を M9「計測と構造化」(ADR-006)へ更新。単元展開は一時停止し、検証・構造化・流通へ重心移動。 |
