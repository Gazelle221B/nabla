...
> **✅ 2026-07-14: 全マイルストーン完走(MVP 1〜3+M4〜M8)。** MVP 2(PR #37〜#39: フーリエ〔計測により Tier 1〕・LLN/CLT〔Pixi〕・フラクタル〔Canvas2D〕)、MVP 3(PR #41〜#44: 一次変換3×3・回転と基底変換・曲面と偏微分・ドメインカラーリング〔ShaderMaterial、Tier 3b 不要を実測確定〕)。公開33単元。
...
> **✅ 2026-07-24: 全マイルストーン完走を受け、次フェーズは M9「計測と構造化」(ADR-006、Issue #49)。** 重心を「生産」から「検証・構造化・流通」へ移す。M9a(GA4 計測基盤+事前登録基準)→ M9b(前提チェック関門パイロット)→ M9c(演習+予想履歴)→ M9d(既定学習経路+教師向け流通)。**M9 期間中、新単元の追加は前提グラフの空洞埋め・経路整備に必要な場合に限定する(単元数を KPI にしない)。** 旧次アクション(M4 着手以降)は完了済みのため歴史的記録として下記に残す。
