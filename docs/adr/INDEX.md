...
| [ADR-005](ADR-005.md) | Tier 3a(標準3D)の導入方式 — vanilla Three.js + useEffect | Accepted(2026-07-14、技術スパイク+人間 Go 済み) | 実装比較スパイクにもとづき R3F 不採用(+103KB に見合う便益なし——キーボード代替の難度は両案同一)。Pixi 前例と同型のライフサイクル規律、preserveDrawingBuffer 規約、離散カメラボタンの a11y 方針、ドメインカラーリングは「まず ShaderMaterial」の Tier 3b 判断枠組み |
