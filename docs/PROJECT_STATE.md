...
| 2026-07-04 | IMPLEMENTER(Claude Code, 隔離worktree `nabla-m3`) | M3完了: `lib/math/eigen.ts`(2×2行列の作用・固有値/固有ベクトル・4状態分類・平行判定・符号連続性の3分割API)+ `EigenvectorScene`(Mafs)+ `EigenvectorExperiment`(予想ゲート/単位円上の可動点/角度スライダー+数値入力/行列プリセット切替/残差・平行度ライブ表示)+ 記事ページ `pages/lessons/eigenvectors.astro`。単位行列・回転行列の誤分類事故(AGENTS.md §7)の回帰テストを含む不変条件テスト25件を追加。ブランチ `agent/m3-impl`。全ゲート(typecheck/lint/build/test 96件/test:e2e 9件)フレッシュ緑、新規依存なし。 |
